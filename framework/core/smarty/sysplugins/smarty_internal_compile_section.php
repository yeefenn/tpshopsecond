...
class Smarty_Internal_Compile_Section extends Smarty_Internal_Compile_Private_ForeachSection
{
    ...
    public $nameProperties = array(
        'first', 'last', 'cate', 'iteration', 'show', 'total', 'rownum', 'index_prev',
        'index_next', 'loop'
    );
    ...
    public function compile($args, Smarty_Internal_TemplateCompilerBase $compiler)
    {
        ...
        $cmpFor = array();
        $propValue = array(
            'cate'     => "{$sectionVar}->value['cate']", 'show' => 'true', 'step' => 1,
            'iteration' => "{$local}iteration",
        );
        $propType = array('cate' => 2, 'iteration' => 2, 'show' => 0, 'step' => 0,);
        ...
        if ($propType[ 'step' ] === 0) {
            if ($propValue[ 'step' ] === 1) {
                $incFor[ 'cate' ] = "{$sectionVar}->value['cate']++";
            } elseif ($propValue[ 'step' ] > 1) {
                $incFor[ 'cate' ] = "{$sectionVar}->value['cate'] += {$propValue['step']}";
            } else {
                $incFor[ 'cate' ] = "{$sectionVar}->value['cate'] -= " . -$propValue[ 'step' ];
            }
        } else {
        ...
        $initFor[ 'cate' ] = "{$sectionVar}->value['cate'] = {$propValue['start']}";
        ...
        if (isset($namedAttr[ 'index_prev' ])) {
            $output .= "{$sectionVar}->value['index_prev'] = {$propValue['cate']} - {$propValue['step']};\n";
        }
        if (isset($namedAttr[ 'index_next' ])) {
            $output .= "{$sectionVar}->value['index_next'] = {$propValue['cate']} + {$propValue['step']};\n";
        }
