...
class Captcha{
    ...
    //获取字符的验证码， 用于保存在服务器中
    function getcode() {
        //$_SESSION['code']=["code"=>"$this->code","time"=>time()];
    ...
    private function createcode() {
        ...
        //$_SESSION['code']=["code"=>"$code","time"=>time()];
...
//        var_dump($session_code);
        //删除
        Session::delete('code');
        //判段
        if($session_code){
            if(time()-$session_code['time']>60){
                return false;
            }
            if(strtolower($code)==strtolower($session_code['code'])){
                return true;
            }else{
                //echo strtolower($session_code['code']);
                return false ;
            }
        }else{
            return false;
